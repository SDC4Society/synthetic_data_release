...
class Mechanism:
    ...
    def gaussian_noise_scale(self, l2_sensitivity, epsilon, delta):
        ...
        if self.bounded:
            l2_sensitivity *= 2.0
        if privacy_calibrator is not None:
            try:
                return (
                    l2_sensitivity
                ...
                pass
        # Fallback to a standard Gaussian mechanism bound when autodp is unavailable.
        assert delta > 0, "delta must be > 0 for Gaussian noise calibration"
