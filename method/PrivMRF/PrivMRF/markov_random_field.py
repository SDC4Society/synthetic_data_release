...
class MarkovRandomField:
    ...
    def generte_noise_scale(self):
        ...
        if self.config['use_exp_mech'] > 0:
            total_privacy_budget = 2 * self.config['rho']*(1-self.config['use_exp_mech'])
        else:
            total_privacy_budget = 2 * self.config['rho']
    ...
    def dump_entropy(self, path, entropy, i):
        ...
        rho = str(self.config['rho'])
        if rho not in entropy_log[self.config['data']]:
            entropy_log[self.config['data']][rho] = {}
        entropy_log[self.config['data']][rho][str(i)] = entropy
    ...
    def select_measure_exp(self, measure_list, measure_num):
        ...
        result_list = []
        budget = self.config['rho']/self.ed_step_num * self.config['use_exp_mech']/measure_num
        for i in range(measure_num):
    ...
    def syn(self, n_sample, preprocesser, path):
        self.n_samples = n_sample
        data = np.zeros((self.n_samples, len(self.domain)), dtype=int)
    ...
    def pandas_generate_cond_column_data(self, clique_factor, cond, target):
        ...
        if len(cond) == 0:
            ...
            self.df.loc[:, target] = tools.generate_column_data(prob, self.n_samples)
        else:
