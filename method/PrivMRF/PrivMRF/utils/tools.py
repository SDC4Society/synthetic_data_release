...
def exponential_mechanism(item_list, score_list, rho, sensitivity):
    debug_list = list(zip(item_list, score_list))

    score_list = np.array(score_list)
    score_list = score_list*np.sqrt(8*rho)/2/sensitivity
    ...
    for i in range(len(debug_list)):
        ...
        if item[0] == item_list[choice]:
            ...
            print('        select: {:d} total: {:d} score: {:.2e} max_score: {:.2e} budget/sensitivity: {:.2e}'\
                .format(i, len(debug_list), item[1], debug_list[0][1], np.sqrt(8*rho)/sensitivity))
            break
