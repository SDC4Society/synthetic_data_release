...
import numpy as np
from method.TabDDPM.model.utils import *
...
class GaussianMultinomialDiffusion(torch.nn.Module):
    ...
    def q_posterior(self, log_x_start, log_x_t, t):
        ...
        # This is a numerical threshold, to make the log_prob tensor within a reasonable range 
        log_prob_max_threshold = np.log(0.99999)
        log_prob_min_threshold = np.log(1e-30)
        log_EV_xtmin_given_xt_given_xstart = torch.clamp(log_EV_xtmin_given_xt_given_xstart, max=log_prob_max_threshold, min=log_prob_min_threshold)
    ...
    def compute_Lt(self, model_out, log_x_start, log_x_t, t, out_dict, detach_mean=False):
        ...
        # print('max log_true_prob:', torch.max(log_true_prob))
        # print('min log_true_prob:', torch.min(log_true_prob))

        # print('max log_model_prob:', torch.max(log_model_prob))
        # print('min log_model_prob:', torch.min(log_model_prob))
    ...
    def sample_all(self, num_samples, batch_size, y_dist, ddim=False):
        ...
        while num_generated < num_samples:
            ...
            num_generated += sample.shape[0] #sample batch_size samples each time
