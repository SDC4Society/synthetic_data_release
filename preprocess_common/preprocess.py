...
class discretizer():
    ...
    def inverse_transform(self, data):
        ...
        if self.ord_encoder is not None:
            for i in range(decoded_data.shape[1]):
                n_categories = len(self.ord_encoder.categories_[i])
                decoded_data[:, i] = np.clip(np.round(decoded_data[:, i]), 0, n_categories - 1)
            decoded_data = self.ord_encoder.inverse_transform(decoded_data)
